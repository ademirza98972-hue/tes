...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. Coba auto-join server (pakai guilds.join scope)
    // Kalau gagal tidak apa-apa, user tetap bisa login
    // Tapi kita cek dulu apakah sudah member
    ...
    if (!isMember) {
      // Redirect ke invite server Discord
      return res.redirect('https://discord.gg/saNDWMhVt9');
    }

    // 4. Simpan/update user di Supabase
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. Cek apakah user member server
